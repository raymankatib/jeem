...
const valueKeys = ["quality", "speed", "communication", "ownership"];
...
export function ProofSection() {
	...
	const testimonials = t("proof.testimonials", { returnObjects: true }) as Array<{ quote: string; role: string }>;
			...
			<div className="container-narrow">
				<motion.div initial="hidden" animate={isInView ? "visible" : "hidden"} variants={staggerContainer}>
					...
					<motion.div variants={fadeInUp} className="max-w-2xl mb-20">
						<p className="text-sm text-muted-foreground uppercase tracking-wider mb-3">{t("proof.sectionLabel")}</p>
						<h2 className="text-3xl sm:text-4xl font-semibold tracking-tight mb-4">{t("proof.title")}</h2>
						<p className="text-muted-foreground text-lg leading-relaxed">{t("proof.description")}</p>
					</motion.div>
							...
							return (
								...
								>
									<div className="h-full bg-card border border-border rounded-2xl p-6 lg:p-8 shadow-soft text-center">
										<h3 className="text-lg font-medium mb-2">{t(`proof.values.${valueKey}.title`)}</h3>
										<p className="text-sm text-muted-foreground">{t(`proof.values.${valueKey}.description`)}</p>
									</div>
								</motion.div>
							);
						})}
						...
						<div className="bg-card border border-border rounded-2xl p-8 lg:p-10 shadow-soft">
							<div className="flex flex-col lg:flex-row lg:items-center justify-between gap-8">
								<div className="max-w-md">
									<h3 className="text-lg font-medium mb-2">{t("proof.standards.title")}</h3>
									<p className="text-sm text-muted-foreground">{t("proof.standards.description")}</p>
								</div>
